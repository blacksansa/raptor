...
import { listProjects, createProject as sbCreateProject, deleteProject as sbDeleteProject } from '../sc/integrations/supabase/projects'
...
export default function ClientHome(): JSX.Element {
  ...
  async function addProject() {
    ...
    try {
      ...
      if (typeof window !== 'undefined') alert && alert('Erro ao salvar no Supabase')
    }
  ...
  async function removeProject(id: number) {
    try {
      ...
      if (typeof window !== 'undefined') alert && alert('Erro ao remover no Supabase')
    }
    setProjects(prev => prev.filter(p => p.id !== id))
  }
  ...
  return (
    <main style={{ padding: 20 }}>
      <h1>Portfólio — Client</h1>

      <div style={{ marginBottom: 12 }}>
        <input placeholder="https://seu-projeto.com" value={url} onChange={e => setUrl(e.target.value)} style={{ width: 320, marginRight: 8 }} />
        <input placeholder="Nome do projeto" value={title} onChange={e => setTitle(e.target.value)} style={{ width: 200, marginRight: 8 }} />
      ...
      <ul>
        {projects.map(p => (
          <li key={p.id} style={{ marginBottom: 8 }}>
            ...
            <button style={{ marginLeft: 8 }} onClick={() => removeProject(p.id)}>Remover</button>
          </li>
        ))}
      </ul>
    </main>
...
export default function ClientHome(): JSX.Element {
  ...
  const [embeddable, setEmbeddable] = useState<Record<number, boolean | null>>({})
  ...
  // check embeddability for each project so we don't render iframes that will be blocked
  useEffect(() => {
    ...
    const checkAll = async () => {
      ...
      for (const p of projects) {
        try {
          const res = await fetch(`/api/check-embed?url=${encodeURIComponent(p.url)}`, { signal: controller.signal })
          const data = await res.json()
          next[p.id] = !!data?.embeddable
        } catch (e) {
          next[p.id] = false
        }
      }
    ...
    if (projects.length) void checkAll()
